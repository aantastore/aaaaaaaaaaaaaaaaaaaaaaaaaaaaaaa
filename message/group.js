...
module.exports = welcome = async (Devil, anu) => {
		...
		try {
            ...
            if (!welkom.includes(anu.jid)) return
               ...
               buff = await getBuffer(pic)
               Devil.sendMessage(mdata.id, { contentText: `${out}`, footerText: `Welcome Message By ${setting.botName}`, buttons: [{buttonId: `!bay`,buttonText:{displayText: 'BYE KAK👋'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
            }
		} catch (e) {
